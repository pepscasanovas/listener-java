 ...
 * <p>
 * A sample java.util.logging properties file - jsr47min.properties is provided
 * that demonstrates how to run with a memory based trace facility that runs
 * with minimal performance overhead. The memory buffer can be dumped when a
 * log/trace record is written matching the MemoryHandlers trigger level or when
 * the push method is invoked on the MemoryHandler.
 * {@link org.eclipse.paho.client.mqttv3.util.Debug Debug} provides method to
 * make it easy to dump the memory buffer as well as other useful debug info.
 */
public class JSR47Logger implements Logger {
	 ...
	 * 
	 * @param logMsgCatalog
	 *            The resource bundle associated with this logger
	 * @param loggerID
	 *            The suffix for the loggerName (will be appeneded to
	 *            org.eclipse.paho.client.mqttv3
	 * @param resourceContext
	 *            A context for the logger e.g. clientID or appName...
	 */
	public void initialise(ResourceBundle logMsgCatalog, String loggerID, String resourceContext) {
		...
		// loggerName = "org.eclipse.paho.client.mqttv3." + ((null == loggerID
		// || 0 == loggerID.length()) ? "internal" : loggerID);
		loggerName = loggerID;
	...
	public void log(int level, String sourceClass, String sourceMethod, String msg, Object[] inserts,
			Throwable thrown) {
		// InternalTracer.log(this.catalogID, level, sourceClass, sourceMethod,
		// msg, inserts, thrown);
		java.util.logging.Level julLevel = mapJULLevel(level);
		if (julLogger.isLoggable(julLevel)) {
			logToJsr47(julLevel, sourceClass, sourceMethod, this.catalogID, this.logMessageCatalog, msg, inserts,
					thrown);
		}
	}
	...
	public void trace(int level, String sourceClass, String sourceMethod, String msg, Object[] inserts, Throwable ex) {
		...
		// if (FINE == level || isJULLoggable ||
		// InternalTracer.isLoggable(level)) {
		// InternalTracer.traceForced(level, sourceClass, sourceMethod, msg,
		// inserts);
		// }
		if (isJULLoggable) {
	...
	private void logToJsr47(java.util.logging.Level julLevel, String sourceClass, String sourceMethod,
			String catalogName, ResourceBundle messageCatalog, String msg, Object[] inserts, Throwable thrown) {
