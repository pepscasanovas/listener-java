...
public class SSLSocketFactoryFactory {
	...
	public static final String DEFAULT_PROTOCOL = "TLS"; // "SSL_TLS" is not
															// supported by
															// DesktopEE

	private static final String propertyKeys[] = { SSLPROTOCOL, JSSEPROVIDER, KEYSTORE, KEYSTOREPWD, KEYSTORETYPE,
			KEYSTOREPROVIDER, KEYSTOREMGR, TRUSTSTORE, TRUSTSTOREPWD, TRUSTSTORETYPE, TRUSTSTOREPROVIDER, TRUSTSTOREMGR,
			CIPHERSUITES, CLIENTAUTH };

	private Hashtable configs; // a hashtable that maps configIDs to properties.

	private Properties defaultProperties;

	private static final byte[] key = { (byte) 0x9d, (byte) 0xa7, (byte) 0xd9, (byte) 0x80, (byte) 0x05, (byte) 0xb8,
			(byte) 0x89, (byte) 0x9c };
	...
	public static String[] unpackCipherSuites(String ciphers) {
		...
		// add last element after the comma or only element if no comma is
		// present.
	...
	/**
	 * @return Returns the set of configuration IDs that exist in the
	 *         SSLSocketFactoryFactory.
	 */
	...
	// *
	// * @return the XPD Keystore if running on the XPD platform (otherwise
	// null).
	...
	// try {
	// Class secPlatClass =
	// Class.forName("com.ibm.rcp.security.auth.SecurePlatform");
	...
	private SSLContext getSSLContext(String configID) throws MqttSecurityException {
		...
		if (logger != null) {
			...
			logger.fine(CLASS_NAME, METHOD_NAME, "12000",
					new Object[] { configID != null ? configID : "null (broker defaults)", protocol });
		}
		...
		try {
			...
			if (logger != null) {
				...
				logger.fine(CLASS_NAME, METHOD_NAME, "12001", new Object[] {
						configID != null ? configID : "null (broker defaults)", ctx.getProvider().getName() });
			}
			...
			// if (logger != null) {
			// if (keyStore == null) {
			// // 12002 "SSL initialization: configID = {0}, XPD keystore not
			// available"
			// logger.fine(CLASS_NAME, METHOD_NAME, "12002", new
			// Object[]{configID!=null ? configID : "null (broker defaults)"});
			// } else {
			// // 12003 "SSL initialization: configID = {0}, XPD keystore
			// available"
			// logger.fine(CLASS_NAME, METHOD_NAME, "12003", new
			// Object[]{configID!=null ? configID : "null (broker defaults)"});
			// }
			// }
			// }
			...
			if (keyStore == null) {
				...
				if (logger != null) {
					// 12004 "SSL initialization: configID = {0}, keystore =
					// {1}"
					logger.fine(CLASS_NAME, METHOD_NAME, "12004",
							new Object[] { configID != null ? configID : "null (broker defaults)",
									keyStoreName != null ? keyStoreName : "null" });
				}

				char[] keyStorePwd = getKeyStorePassword(configID);
				if (logger != null) {
					// 12005 "SSL initialization: configID = {0}, keystore
					// password = {1}"
					logger.fine(CLASS_NAME, METHOD_NAME, "12005",
							new Object[] { configID != null ? configID : "null (broker defaults)",
				...
				if (logger != null) {
					// 12006 "SSL initialization: configID = {0}, keystore type
					// = {1}"
					logger.fine(CLASS_NAME, METHOD_NAME, "12006",
							new Object[] { configID != null ? configID : "null (broker defaults)",
				...
				if (keyStoreName != null && keyStoreType != null && keyMgrAlgo != null) {
					try {
						...
						if (logger != null) {
							// 12010 "SSL initialization: configID = {0},
							// keystore manager algorithm = {1}"
							logger.fine(CLASS_NAME, METHOD_NAME, "12010",
									new Object[] { configID != null ? configID : "null (broker defaults)",
											keyMgrAlgo != null ? keyMgrAlgo : "null" });
							// 12009 "SSL initialization: configID = {0},
							// keystore manager provider = {1}"
							logger.fine(CLASS_NAME, METHOD_NAME, "12009",
									new Object[] { configID != null ? configID : "null (broker defaults)",
			...
			if (logger != null) {
				...
				logger.fine(CLASS_NAME, METHOD_NAME, "12011",
						new Object[] { configID != null ? configID : "null (broker defaults)",
			...
			if (logger != null) {
				// 12012 "SSL initialization: configID = {0}, truststore
				// password = {1}"
				logger.fine(CLASS_NAME, METHOD_NAME, "12012",
						new Object[] { configID != null ? configID : "null (broker defaults)",
			...
			if (logger != null) {
				// 12013 "SSL initialization: configID = {0}, truststore type =
				// {1}"
				logger.fine(CLASS_NAME, METHOD_NAME, "12013",
						new Object[] { configID != null ? configID : "null (broker defaults)",
			...
			if (trustStoreName != null && trustStoreType != null && trustMgrAlgo != null) {
				try {
					...
					if (logger != null) {
						...
						// 12017 "SSL initialization: configID = {0}, truststore
						// manager algorithm = {1}"
						logger.fine(CLASS_NAME, METHOD_NAME, "12017",
								new Object[] { configID != null ? configID : "null (broker defaults)",
										trustMgrAlgo != null ? trustMgrAlgo : "null" });

						// 12016 "SSL initialization: configID = {0}, truststore
						// manager provider = {1}"
						logger.fine(CLASS_NAME, METHOD_NAME, "12016",
								new Object[] { configID != null ? configID : "null (broker defaults)",
	...
	// *
	// * @see
	// org.eclipse.paho.client.mqttv3.internal.security.SSLSocketFactoryFactory#DEFAULT_PROTOCOL
	// *
	// * @param configID
	...
	// if (logger != null) {
	// // 12018 "SSL initialization: configID = {0}, application-enabled cipher
	// suites = {1}"
	// logger.fine(CLASS_NAME, METHOD_NAME, "12018", new Object[]{configID!=null
	// ? configID : "null (broker defaults)",
	// getEnabledCipherSuites(configID)!=null ? getProperty(configID,
	// CIPHERSUITES, null) : "null (using platform-enabled cipher suites)"});
	//
	// // 12019 "SSL initialization: configID = {0}, client authentication =
	// {1}"
	// logger.fine(CLASS_NAME, METHOD_NAME, "12019", new Object[]{configID!=null
	// ? configID : "null (broker defaults)",
	...
	public SSLSocketFactory createSocketFactory(String configID) throws MqttSecurityException {
		...
		SSLContext ctx = getSSLContext(configID);
		if (logger != null) {
			// 12020 "SSL initialization: configID = {0}, application-enabled
			// cipher suites = {1}"
			logger.fine(CLASS_NAME, METHOD_NAME, "12020",
					new Object[] { configID != null ? configID : "null (broker defaults)",
							getEnabledCipherSuites(configID) != null ? getProperty(configID, CIPHERSUITES, null)
									: "null (using platform-enabled cipher suites)" });
		}
