...
/**
 * An implementation of the {@link MqttClientPersistence} interface that
 * provides file based persistence.
 * 
 * A directory is specified when the Persistence object is created. When the
 * persistence is then opened (see {@link #open(String, String)}), a
 * sub-directory is made beneath the base for this client ID and connection key.
 * This allows one persistence base directory to be shared by multiple clients.
 * 
 * The sub-directory's name is created from a concatenation of the client ID and
 * connection key with any instance of '/', '\\', ':' or ' ' removed.
 */
public class MqttDefaultFilePersistence implements MqttClientPersistence {
	 ...
	 */
	public MqttDefaultFilePersistence(String directory) { // throws
	...
	public void open(String clientId, String theConnection) throws MqttPersistenceException {
		...
		synchronized (this) {
			...
			try {
				...
				// throw new
				// MqttPersistenceException(MqttPersistenceException.REASON_CODE_PERSISTENCE_IN_USE);
			}
