 ...
 * multiple threads. When a message is sent, a token is associated with the
 * message and saved using the {@link #saveToken(MqttToken, MqttWireMessage)}
 * method. Anyone interested in tacking the state can call one of the wait
 * methods on the token or using the asynchronous listener callback method on
 * the operation. The {@link CommsReceiver} class, on another thread, reads
 * responses back from the network. It uses the response to find the relevant
 * token, which it can then notify.
 * 
 * Note: Ping, connect and disconnect do not have a unique message id as only
 * one outstanding request of each type is allowed to be outstanding
 */
public class CommsTokenStore {
	...
	/**
	 * Restores a token after a client restart. This method could be called for
	 * a SEND of CONFIRM, but either way, the original SEND is what's needed to
	 * re-build the token.
	 */
