...
public class MqttMessage {
	 ...
	 * the message was from a current publisher, or was "retained" by the server
	 ...
	 * engine. Sending a message with retained set to <code>true</code> and with
	 * an empty byte array as the payload e.g. <code>new byte[0]</code> will
	 * clear the retained message from the server. The default value is
	 * <code>false</code>
	 *
	 * @param retained
	 ...
	 * an authorization problem), then an
	 * {@link MqttCallback#deliveryComplete(IMqttDeliveryToken)}. Also known as
	 * "fire and forget".</li>
	 *
	 ...
	 * persistence is not configured then delivery of QoS 1 and 2 messages can
	...
	/**
	 * Returns a string representation of this message's payload. Makes an
	 * attempt to return the payload as a string. As the MQTT client has no
	 * control over the content of the payload it may fail.
	 * 
	 ...
	 * 
	 * @param mutable
	 *            <code>true</code> if the values can be changed,
