...
public class Token {
	...
	/**
	 * Waits for the message delivery to complete, but doesn't throw an
	 * exception in the case of a NACK. It does still throw an exception if
	 * something else goes wrong (e.g. an IOException). This is used for packets
	 * like CONNECT, which have useful information in the ACK that needs to be
	 * accessed.
	 */
	...
	protected MqttWireMessage waitForResponse(long timeout) throws MqttException {
		...
		synchronized (responseLock) {
			// @TRACE 400=>key={0} timeout={1} sent={2} completed={3}
			// hasException={4} response={5} token={6}
			log.fine(CLASS_NAME,
					methodName, "400", new Object[] { getKey(), new Long(timeout), new Boolean(sent),
	 ...
	 * 
	 * @param msg
	 *            response message. Optional - there are no response messages
	 *            for some flows
	 * @param ex
	...
	protected void markComplete(MqttWireMessage msg, MqttException ex) {
		...
		synchronized (responseLock) {
			// ACK means that everything was OK, so mark the message for garbage
			// collection.
