...
public class ClientState {
	...
	private static final int MIN_MSG_ID = 1; // Lowest possible MQTT message ID
												// to use
	private static final int MAX_MSG_ID = 65535; // Highest possible MQTT
													// message ID to use
	private int nextMsgId = MIN_MSG_ID - 1; // The next available message ID to
											// use
	...
	/**
	 * Inserts a new message to the list, ensuring that list is ordered from
	 * lowest to highest in terms of the message id's.
	 * 
	 * @param list
	 ...
	 * 
	 * @param list
	 *            the list containing the messages to produce a new reordered
	 *            list for - this will not be modified or replaced, i.e., be
	 *            read-only to this method
	...
	private Vector reOrder(Vector list) {
		...
		// we need to check that the gap after highest msg id to the lowest msg
		// id is not beaten
			...
			if (message != null) {
				if (key.startsWith(PERSISTENCE_RECEIVED_PREFIX)) {
					...
					if (persistence.containsKey(getSendConfirmPersistenceKey(sendMessage))) {
						...
						// NO DUP flag is allowed for 3.1.1 spec while it's not
						// clear for 3.1 spec
						...
						if (confirmMessage != null) {
							...
							// @TRACE 605=outbound QoS 2 pubrel key={0}
							// message={1}
						...
						if (sendMessage.getMessage().getQos() == 2) {
							...
							// @TRACE 608=outbound QoS 1 publish key={0}
							// message={1}
					...
					// Buffered outgoing messages that have not yet been sent at
					// all
					MqttPublish sendMessage = (MqttPublish) message;
					highestMsgId = Math.max(sendMessage.getMessageId(), highestMsgId);
					if (sendMessage.getMessage().getQos() == 2) {
						...
						// Because there is no Puback, we have to trust that
						// this is enough to send the message
						persistence.remove(key);
	...
	private void restoreInflightMessages() {
		...
		while (keys.hasMoreElements()) {
			...
			if (msg instanceof MqttPublish) {
				...
				// set DUP flag only for PUBLISH, but NOT for PUBREL (spec
				// 3.1.1)
	...
	public void send(MqttWireMessage message, MqttToken token) throws MqttException {
		...
		if (message instanceof MqttPublish) {
			...
			if (message instanceof MqttConnect) {
				synchronized (queueLock) {
					// Add the connect action at the head of the pending queue
					// ensuring it jumps
	...
	public void persistBufferedMessage(MqttWireMessage message) {
		...
		try {
			...
			try {
				...
				// @TRACE 515=Could not Persist, attempting to Re-Open
				// Persistence Store
				log.fine(CLASS_NAME, methodName, "515");
				// TODO - Relies on
				// https://github.com/eclipse/paho.mqtt.java/issues/178
				persistence.open(this.clientComms.getClient().getClientId(),
						this.clientComms.getClient().getClientId());
				persistence.put(key, (MqttPublish) message);
			}
	...
	/**
	 * This removes the MqttSend message from the outbound queue and
	 * persistence.
	 * 
	 * @param message
	 * @throws MqttPersistenceException
	...
	/**
	 * Check and send a ping if needed and check for ping timeout. Need to send
	 * a ping if nothing has been sent or received in the last keepalive
	 * interval. It is important to check for both sent and received packets in
	 * order to catch the case where an app is solely sending QoS 0 messages or
	 * receiving QoS 0 messages. QoS 0 message are not good enough for checking
	 * a connection is alive as they are one way messages.
	 * 
	 * If a ping has been sent but no data has been received in the last
	 * keepalive interval then the connection is deamed to be broken.
	 * 
	...
	public MqttToken checkForActivity(IMqttActionListener pingCallback) throws MqttException {
		...
		if (connected && this.keepAlive > 0) {
			...
			// Reduce schedule frequency since System.currentTimeMillis is no
			// accurate, add a buffer
			...
			synchronized (pingOutstandingLock) {
				...
				// Is the broker connection lost because the broker did not
				// reply to my ping?
				if (pingOutstanding > 0 && (time - lastInboundActivity >= keepAlive + delta)) {
					// lastInboundActivity will be updated once receiving is
					// done.
					// Add a delta, since the timer and
					// System.currentTimeMillis() is not accurate.
					// A ping is outstanding but no packet has been received in
					// KA so connection is deemed broken
					// @TRACE 619=Timed out as no activity, keepAlive={0}
					// lastOutboundActivity={1} lastInboundActivity={2} time={3}
					// lastPing={4}
					...
					// A ping has already been sent. At this point, assume that
					// the
				...
				// Is the broker connection lost because I could not get any
				// successful write for 2 keepAlive intervals?
				if (pingOutstanding == 0 && (time - lastOutboundActivity >= 2 * keepAlive)) {

					// I am probably blocked on a write operations as I should
					// have been able to write at least a ping message
					log.severe(CLASS_NAME, methodName, "642",
							new Object[] { new Long(this.keepAlive), new Long(lastOutboundActivity),
									new Long(lastInboundActivity), new Long(time), new Long(lastPing) });

					// A ping has not been sent but I am not progressing on the
					// current write operation.
					// At this point, assume that the broker has hung and the
					// TCP layer hasn't noticed.
					throw ExceptionHelper.createMqttException(MqttException.REASON_CODE_WRITE_TIMEOUT);
				}

				// 1. Is a ping required by the client to verify whether the
				// broker is down?
				// Condition: ((pingOutstanding == 0 && (time -
				// lastInboundActivity >= keepAlive + delta)))
				// In this case only one ping is sent. If not confirmed, client
				// will assume a lost connection to the broker.
				...
				// the broker needs to keep receiving a regular ping even if the
				// ping response are queued after the long message
				// If lacking to do so, the broker will consider my connection
				// lost and cut my socket.
				if ((pingOutstanding == 0 && (time - lastInboundActivity >= keepAlive - delta))
					...
					// pingOutstanding++; // it will be set after the ping has
					// been written on the wire
					// lastPing = time; // it will be set after the ping has
					// been written on the wire
					...
					// Wake sender thread since it may be in wait state (in
					// ClientState.get())
					notifyQueueLock();
				} else {
	...
	/**
	 * This returns the next piece of work, ie message, for the CommsSender to
	 * send over the network. Calls to this method block until either: - there
	 * is a message to be sent - the keepAlive interval is exceeded, which
	 * triggers a ping message to be returned -
	...
	protected MqttWireMessage get() throws MqttException {
		...
		synchronized (queueLock) {
			while (result == null) {
				...
				// If the inflight window is full and no flows are pending wait
				...
				if ((pendingMessages.isEmpty() && pendingFlows.isEmpty())
					...
					try {
						// @TRACE 644=wait for new work or for space in the
						// inflight window
				...
				// Handle the case where not connected. This should only be the
				// case if:
				...
				// Check if there is a need to send a ping to keep the session
				// alive.
				// Note this check is done before processing messages. If not
				// done first
				// an app that only publishes QoS 0 messages will prevent
	...
	/**
	 * COMMENTED OUT AS NO LONGER USED. Deduce how long to to wait until a ping
	 * is required.
	 * 
	 * In order to keep the connection alive the server must see activity within
	 * the keepalive interval. If the application is not sending / receiving any
	 * messages then the client will send a ping. This method works out the next
	 * time that a ping must be sent in order for the server to know the client
	 * is alive.
	 * 
	 * @return time before a ping needs to be sent to keep alive the connection
	 *         long getTimeUntilPing() { long pingin = getKeepAlive(); // If KA
	 *         is zero which means just wait for work or // if a ping is
	 *         outstanding return the KA value if (connected && (getKeepAlive()
	 *         > 0) && !pingOutstanding) {
	 * 
	 *         long time = System.currentTimeMillis(); long timeSinceOut =
	 *         (time-lastOutboundActivity); long timeSinceIn =
	 *         (time-lastInboundActivity);
	 * 
	 *         if (timeSinceOut > timeSinceIn) { pingin =
	 *         (getKeepAlive()-timeSinceOut); } else { pingin =
	 *         (getKeepAlive()-timeSinceIn); }
	 * 
	 *         // Unlikely to be negative or zero but in the case it is return a
	 *         // small value > 0 to cause a ping to occur if (pingin <= 0) {
	 *         pingin = 10; } } return (pingin); }
	 */
	...
	protected void notifySent(MqttWireMessage message) {
		...
		if (message instanceof MqttPingReq) {
			...
			if (((MqttPublish) message).getMessage().getQos() == 0) {
				// once a QoS 0 message is sent we can clean up its records
				// straight away as
	...
	/**
	 * Called by the CommsReceiver when a message has been received. Handles
	 * inbound messages and other flows such as PUBREL.
	 * 
	 * @param message
	 * @throws MqttException
	 */
	...
	protected void notifyComplete(MqttToken token) throws MqttException {
		...
		if (message != null && message instanceof MqttAck) {
			...
			if (ack instanceof MqttPubAck) {
				...
				// @TRACE 645=removed QoS 2 publish/pubrel. key={0}, -1
				// inFlightPubRels={1}
	...
	protected void notifyResult(MqttWireMessage ack, MqttToken token, MqttException ex) {
		...
		// Let the user know an async operation has completed and then remove
		// the token
	...
	public void connected() {
		...
		pingSender.start(); // Start ping thread when client connected to
							// server.
	}

	/**
	 * Called during shutdown to work out if there are any tokens still to be
	 * notified and waiters to be unblocked. Notifying and unblocking takes
	 * place after most shutdown processing has completed. The tokenstore is
	 * tidied up so it only contains outstanding delivery tokens which are valid
	 * after reconnect (if clean session is false)
	 * 
	 * @param reason
	 *            The root cause of the disconnection, or null if it is a clean
	 *            disconnect
	 */
	...
	public void disconnected(MqttException reason) {
		...
		try {
			...
			pendingFlows.clear();
			synchronized (pingOutstandingLock) {
				// Reset pingOutstanding to allow reconnects to assume no
				// previous ping.
				pingOutstanding = 0;
			}
	...
	/**
	 * Get the next MQTT message ID that is not already in use, and marks it as
	 * now being in use.
	 * 
	 ...
	 * preventing the callback on any newly received messages. After the timeout
	 * expires, delete any pending messages except for outbound ACKs, and wait
	 * for those ACKs to complete.
	 */
	public void quiesce(long timeout) {
		...
		if (timeout > 0) {
			...
			synchronized (quiesceLock) {
				try {
					// If token count is not zero there is outbound work to
					// process and
					// if pending flows is not zero there is outstanding work to
					// complete and
					// if call back is not quiseced there it needs to complete.
					int tokc = tokenStore.count();
					if (tokc > 0 || pendingFlows.size() > 0 || !callback.isQuiesced()) {
						// @TRACE 639=wait for outstanding: actualInFlight={0}
						// pendingFlows={1} inFlightPubRels={2} tokens={3}
						...
						// wait for outstanding in flight messages to complete
						// and
			...
			// Quiesce time up or inflight messages delivered. Ensure pending
			// delivery
			// vectors are cleared ready for disconnect to be sent as the final
			// flow.
			synchronized (queueLock) {
				pendingMessages.clear();
				pendingFlows.clear();
