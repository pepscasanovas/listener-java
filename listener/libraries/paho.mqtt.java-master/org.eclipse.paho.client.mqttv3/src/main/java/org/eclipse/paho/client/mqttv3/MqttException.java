...
public class MqttException extends Exception {
	...
	/**
	 * Client encountered an exception. Use the {@link #getCause()} method to
	 * get the underlying reason.
	 */
	...
	/**
	 * Client timed out while waiting for a response from the server. The server
	 * is no longer responding to keep-alive messages.
	 */
	 ...
	 * This can occur when waiting on a token, and then disconnecting the
	 * client. If the message delivery does not complete within the quiesce
	 * timeout period, then the waiting token will be notified with an
	 * exception.
	 */
	...
	/**
	 * The client is not connected to the server. The
	 * {@link MqttClient#connect()} or
	 * {@link MqttClient#connect(MqttConnectOptions)} method must be called
	 ...
	 * <code>javax.net.SocketFactory</code>, and URIs beginning
	 * <code>ssl://</code> must use a
	 ...
	 * made from within a method on {@link MqttCallback}. These methods are
	 ...
	 * another action. If the action is complete the reset() can ve called on
	 ...
	 * inflight messages has already been reached. Once one or more messages
	 * have been moved then new messages can be sent.
	 */
