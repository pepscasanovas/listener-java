...
/**
 * Enables an application to communicate with an MQTT server using blocking
 * methods.
 * <p>
 ...
 * the application program once the MQTT action has completed.</li>
 ...
 * block the application programs thread. This non blocking approach is handy
 * when the application wants to carry on processing while the MQTT action takes
 ...
 * while the connect action is occurring. Non-blocking methods are particularly
 * useful in event-oriented programs and graphical programs where issuing
 * methods that take time to complete on the the main or GUI thread can cause
 * problems.</li>
 * </ul>
 * </p>
 * <p>
 * The non-blocking client can also be used in a blocking form by turning a
 * non-blocking method into a blocking invocation using the following pattern:
 * <code><pre>
 *     IMqttToken token;
 *     token = asyncClient.method(parms).waitForCompletion();
 *     </pre></code> Using the non-blocking client allows an application to use
 * a mixture of blocking and non-blocking styles. Using the blocking client only
 * allows an application to use one style. The blocking client provides
 * compatibility with earlier versions of the MQTT client.
 * </p>
 */
public interface IMqttClient { // extends IMqttAsyncClient {
	 ...
	 *             when the server rejects the connect for security reasons
	 * @throws MqttException
	 *             for non security related problems including communication
	 *             errors
	 */
	 ...
	 * @param options
	 *            a set of connection parameters that override the defaults.
	 * @return the MqttToken used for the call. Can be used to obtain the
	 ...
	 * MQTT server and it will certainly fail to send the disconnect packet. It
	 * will wait for a maximum of 30 seconds for work to quiesce before
	 * disconnecting and wait for a maximum of 10 seconds for sending the
	 ...
	 * levels within a topic. For example, if you subscribe to
	 * <span><span class="filepath">finance/stock/ibm/#</span></span>, you
	 ...
	 * delivered.
	 * <p>
	 * This is a convenience method, which will create a new {@link MqttMessage}
	 ...
	 * <p>
	 * An alternative method that should be used in preference to this one when
	 * publishing a message is:
	 * <ul>
	 * <li>{@link MqttClient#publish(String, MqttMessage)} to publish a message
	 * in a blocking manner
	 * <li>or use publish methods on the non-blocking client like
	 * {@link IMqttAsyncClient#publish(String, MqttMessage, Object, IMqttActionListener)}
	 * </ul>
	 * </p>
	 * <p>
	 * When building an application, the design of the topic tree should take
	 ...
	 * control over when the acks are sent. The default behaviour, when
	 * manualAcks is false, is to send the MQTT acknowledgements automatically
	 * at the successful completion of the messageArrived callback method.
	 * 
	 * @param manualAcks
	 */
	...
	/**
	 * Indicate that the application has completed processing the message with
	 * id messageId. This will cause the MQTT acknowledgement to be sent to the
	 * server.
	 * 
	 * @param messageId
