 ...
 * any network operation) return control to the caller immediately. The
 * operation then proceeds to run in the background so as not to block the
 * invoking thread. An IMqttToken is used to track the state of the operation.
 * An application can use the token to wait for an operation to complete. A
 * token is passed to callbacks once the operation completes and provides
 * context linking it to the original request. A token is associated with a
 * single operation.
 * <p>
 * <p>
...
public interface IMqttToken {
	 ...
	 * outstanding at a time. For other operations the MQTT message id flowed
	 * over the network.
	 */
