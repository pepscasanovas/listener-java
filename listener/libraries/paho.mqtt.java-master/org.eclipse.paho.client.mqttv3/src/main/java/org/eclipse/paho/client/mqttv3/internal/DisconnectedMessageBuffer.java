...
public class DisconnectedMessageBuffer implements Runnable {
	...
	private static final Logger log = (Logger) LoggerFactory.getLogger(LoggerFactory.MQTT_CLIENT_MSG_CAT, CLASS_NAME);
	...
	/**
	 * This will add a new message to the offline buffer, if the buffer is full
	 * and deleteOldestMessages is enabled then the 0th item in the buffer will
	 * be deleted and the new message will be added. If it is not enabled then
	 * an MqttException will be thrown.
	 * 
	 * @param message
	...
	public void run() {
		...
		while (getMessageCount() > 0) {
			try {
				...
				// Error occurred attempting to publish buffered message likely
				// because the client is not connected
				// @TRACE 517=Error occured attempting to publish buffered
				// message due to disconnect.
