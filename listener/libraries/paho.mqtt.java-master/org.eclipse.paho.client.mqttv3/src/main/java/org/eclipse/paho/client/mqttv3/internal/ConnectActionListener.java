...
public class ConnectActionListener implements IMqttActionListener {
	...
	public ConnectActionListener(MqttAsyncClient client, MqttClientPersistence persistence, ClientComms comms,
			MqttConnectOptions options, MqttToken userToken, Object userContext, IMqttActionListener userCallback,
			boolean reconnect) {
	...
	public void onSuccess(IMqttToken token) {
		...
		userToken.internalTok.setClient(this.client); // fix bug 469527 - maybe
														// should be set
														// elsewhere?
	...
	public void onFailure(IMqttToken token, Throwable exception) {
		...
		if ((index + 1) < numberOfURIs || (originalMqttVersion == MqttConnectOptions.MQTT_VERSION_DEFAULT
			...
			userToken.internalTok.notifyComplete();
			userToken.internalTok.setClient(this.client); // fix bug 469527 -
															// maybe should be
															// set elsewhere?
