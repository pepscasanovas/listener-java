...
public abstract class MqttWireMessage {
	...
	public MqttWireMessage(byte type) {
		this.type = type;
		// Use zero as the default message ID. Can't use -1, as that is
		// serialized
	...
	/**
	 * Returns a key associated with the message. For most message types this
	 * will be unique. For connect, disconnect and ping only one message of this
	 * type is allowed so a fixed key will be returned
	 * 
	...
	public static MqttWireMessage createWireMessage(MqttPersistable data) throws MqttException {
		...
		// The persistable interface allows a message to be restored entirely in
		// the header array
	 ...
	 * DataOutputStream the length of the encoded string is encoded into two
	 * bytes and then written to the
	 ...
	 *             Thrown when an error occurs with either the encoding or
	 ...
	 * @throws MqttException
	 *             thrown when an error occurs with either reading from the
