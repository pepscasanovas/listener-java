 ...
 * replaced using
 * {@link org.eclipse.paho.client.mqttv3.logging.LoggerFactory#setLogger(String)}
 * which takes an implementation of the
 * {@link org.eclipse.paho.client.mqttv3.logging.Logger} interface.
 */
public class LoggerFactory {
	...
	/**
	 * Find or create a logger for a named package/class. If a logger has
	 * already been created with the given name it is returned. Otherwise a new
	 * logger is created. By default a logger that uses java.util.logging will
	 * be returned.
	 * 
	 * @param messageCatalogName
	 *            the resource bundle containing the logging messages.
	 * @param loggerID
	...
	public static Logger getLogger(String messageCatalogName, String loggerID) {
		...
		// logger = getJSR47Logger(ResourceBundle.getBundle(messageCatalogName),
		// loggerID, null) ;
	 ...
	 * 
	 * @param the
	 *            class name of the load to load
	 * @param messageCatalog
	 *            the resource bundle containing messages
	 * @param loggerID
	 *            an identifier for the logger
	 * @param resourceName
	 *            a name or context to associate with this logger instance.
	 ...
	 * @return the property value, or null if it isn't set or JSR47 isn't being
	 *         used
	 */
	...
	/**
	 * Set the class name of the logger that the LoggerFactory will load If not
	 * set getLogger will attempt to create a logger appropriate for the
	 * platform.
	 * 
