...
public class MqttConnectOptions {
	...
	/**
	 * The default MqttVersion is 3.1.1 first, dropping back to 3.1 if that
	 * fails
	 */
	 ...
	 *             if the user name is blank or only contains whitespace
	 *             characters.
	 */
	 ...
	 * value of 0 disables keepalive processing in the client.
	 * <p>
	 ...
	 * apply its own policies around the creation of network sockets. If using
	 * an SSL connection, an <code>SSLSocketFactory</code> can be used to supply
	...
	/**
	 * Returns the message to be sent as last will and testament (LWT). The
	 * returned object is "read only". Calling any "setter" methods on the
	 * returned object will result in an <code>IllegalStateException</code>
	 * being thrown.
	 * 
	 ...
	 * (JSSE) is available. These properties are <em>not</em> used if a
	 * SocketFactory has been set using
	 ...
	 * the KeyManager to use. For example /mydir/etc/key.p12</dd>
	 *
	 * <dt>com.ibm.ssl.keyStorePassword</dt>
	 * <dd>The password for the KeyStore object that you want the KeyManager to
	 * use. The password can either be in plain-text, or may be obfuscated using
	 * the static method:
	 ...
	 * <p>
	 * Each <code>serverURI</code> specifies the address of a server that the
	 ...
	 * <p>
	 * If serverURIs is set then it overrides the serverURI parameter passed in
	 * on the constructor of the MQTT client.
	 * <p>
	 ...
	 * "equal" MQTT servers share state. An MQTT client can connect to any of
	 * the "equal" servers and be assured that messages are reliably delivered
	 * and durable subscriptions are maintained no matter which server the
	 * client connects to.
	 * <p>
	 ...
	 * message delivery and durable subscriptions are not valid. The
	 ...
	 * will attempt to reconnect to the server. It will initially wait 1 second
	 * before it attempts to reconnect, for every failed reconnect attempt, the
	 * delay will double until it is at 2 minutes at which point the delay will
	 * stay at 2 minutes.</li>
	 * </ul>
	 * 
