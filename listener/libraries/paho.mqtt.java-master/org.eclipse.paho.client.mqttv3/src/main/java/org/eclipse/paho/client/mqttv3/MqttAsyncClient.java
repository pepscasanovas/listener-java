...
public class MqttAsyncClient implements IMqttAsyncClient { // DestinationProvider
	...
	public IMqttToken connect(MqttConnectOptions options, Object userContext, IMqttActionListener callback)
		...
		comms.setReconnectCallback(new MqttCallbackExtended() {
			...
			public void connectionLost(Throwable cause) {
				if (automaticReconnect) {
					// Automatic reconnect is set so make sure comms is in
					// resting state
		...
		userToken.setActionCallback((IMqttActionListener) connectActionListener);
	 ...
	 * <ul>
	 * <li>{@link MqttAsyncClient#publish(String, MqttMessage, MqttDeliveryToken)}
	 * to publish a message in a non-blocking manner or
	 * <li>{@link MqttClient#publishBlock(String, MqttMessage, MqttDeliveryToken)}
		...
		try {
			connect(this.connOpts, this.userContext, new IMqttActionListener() {
				...
				public void onSuccess(IMqttToken asyncActionToken) {
					...
					log.fine(CLASS_NAME, methodName, "501",
				...
				public void onFailure(IMqttToken asyncActionToken, Throwable exception) {
					...
					log.fine(CLASS_NAME, methodName, "502",
							new Object[] { asyncActionToken.getClient().getClientId() });
