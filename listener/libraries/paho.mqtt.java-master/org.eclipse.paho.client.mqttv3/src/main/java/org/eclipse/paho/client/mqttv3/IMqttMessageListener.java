...
public interface IMqttMessageListener {
	 ...
	 * </p>
	 * <p>
	 * If an implementation of this method throws an <code>Exception</code>,
	 * then the client will be shut down. When the client is next re-connected,
	 * any QoS 1 or 2 messages will be redelivered by the server.
	 * </p>
	 * <p>
	 * Any additional messages which arrive while an implementation of this
	 * method is running, will build up in memory, and will then back up on the
	 * network.
	 * </p>
	 * <p>
	 * If an application needs to persist data, then it should ensure the data
	 ...
	 * callback (for example, a response to this message), but the
	 * implementation must not disconnect the client, as it will be impossible
	 * to send an acknowledgment for the message being processed, and a deadlock
	 * will occur.
	 * </p>
	 *
	 * @param topic
