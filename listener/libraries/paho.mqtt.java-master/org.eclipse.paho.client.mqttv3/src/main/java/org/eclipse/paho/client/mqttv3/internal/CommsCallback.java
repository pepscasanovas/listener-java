...
public class CommsCallback implements Runnable {
				...
				if (running) {
					...
					synchronized (completeQueue) {
						if (!completeQueue.isEmpty()) {
							// First call the delivery arrived callback if
							// needed
					...
					synchronized (messageQueue) {
						if (!messageQueue.isEmpty()) {
							// Note, there is a window on connect where a
							// publish
	...
	public void connectionLost(MqttException cause) {
		...
		try {
			...
			// is called during shutdown processing so no need to do anything
			// else
	...
	public void removeMessageListener(String topicFilter) {
		this.callbacks.remove(topicFilter); // no exception thrown if the filter
											// was not present
	}
	...
	protected boolean deliverMessage(String topicName, int messageId, MqttMessage aMessage) throws Exception {
		...
		/*
		 * if the message hasn't been delivered to a per subscription handler,
		 * give it to the default handler
		 */
