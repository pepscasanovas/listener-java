...
public class TCPNetworkModule implements NetworkModule {
	...
	/**
	 * Constructs a new TCPNetworkModule using the specified host and port. The
	 * supplied SocketFactory is used to supply the network socket.
	 */
	...
	public void start() throws IOException, MqttException {
		...
		try {
			...
			log.fine(CLASS_NAME, methodName, "252",
					new Object[] { host, new Integer(port), new Long(conTimeout * 1000) });
			SocketAddress sockaddr = new InetSocketAddress(host, port);
			socket = factory.createSocket();
			socket.connect(sockaddr, conTimeout * 1000);

			// SetTcpNoDelay was originally set ot true disabling Nagle's
			// algorithm.
			// This should not be required.
			// socket.setTcpNoDelay(true); // TCP_NODELAY on, which means we do
			// not use Nagle's algorithm
